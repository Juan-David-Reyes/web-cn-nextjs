...
// ── Íconos fallback (se usan si el admin no sube un ícono) ───────────────────

const FallbackIcons: Record<number, React.ReactNode> = {
  0: (
...
// ── Defaults (si el admin no ha guardado datos) ──────────────────────────────

const DEFAULT_ITEMS: DiagnosticoItem[] = [
  {
    label: 'Tiempos',
...
export function DiagnosticoSection({ data }: DiagnosticoSectionProps) {
  const preTitle      = data?.preTitle      || 'Diagnóstico';
  const title         = data?.title         || 'Una web lenta o un diseño confuso es una';
  const titleHighlight = data?.titleHighlight || 'fuga de dinero silenciosa.';
      ...
      <div className="w-full px-6 md:px-12 lg:px-[120px] flex flex-col items-center text-center">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-8">
          {items.map((item, index) => (
            <div key={item.id ?? item.label} className="flex flex-col items-start text-left gap-4">
              ...
              >
                {item.icon?.url ? (
                  ...
                  // Fallback inline SVG si aún no se sube ícono
